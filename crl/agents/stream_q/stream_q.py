...
from agents.optim import ObGD as Optimizer
from wrappers.time_wrapper import AddTimeInfo
from agents.normalization_wrappers import NormalizeObservation, ScaleReward
from agents.sparse_init import sparse_init
...
class StreamQ(nn.Module):
    def __init__(
        ...
        self.optimizer = Optimizer(
            list(self.parameters()), lr=lr, gamma=gamma, lamda=lamda, kappa=kappa_value
        )
    ...
    def sample_action(self, s):
        ...
        self.epsilon = linear_schedule(
            self.epsilon_start,
            self.epsilon_target,
            int(self.exploration_fraction * self.total_steps),
            self.time_step,
        )
    ...
    def update_params(
        self, s, a, r, s_prime, done, is_nongreedy, overshooting_info=False
    ):
...
def main(
    ...
    np.random.seed(seed)
    env = (
        gym.make(env_name, render_mode="human", max_episode_steps=10_000)
        if render
        else gym.make(env_name, max_episode_steps=10_000)
    )
    ...
    for t in range(1, total_steps + 1):
        ...
        agent.update_params(
            s, a, r, s_prime, terminated or truncated, is_nongreedy, overshooting_info
        )
        s = s_prime
        if terminated or truncated:
            ...
            episode_num += 1
    env.close()
    save_dir = "data_stream_q_{}_lr{}_gamma{}_lamda{}".format(
        env.spec.id, lr, gamma, lamda
    )
