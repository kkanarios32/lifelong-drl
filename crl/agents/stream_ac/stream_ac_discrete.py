...
class StreamAC(nn.Module):
    def __init__(
        ...
        self.policy_net = Actor(
            n_obs=n_obs, n_actions=n_actions, hidden_size=hidden_size
        )
    ...
    def update_params(
        self, s, a, r, s_prime, done, entropy_coeff, overshooting_info=False
    ):
...
def main(
    ...
    for t in range(1, total_steps + 1):
        ...
        agent.update_params(
            s, a, r, s_prime, terminated or truncated, entropy_coeff, overshooting_info
        )
        s = s_prime
        if terminated or truncated:
            if debug:
                print(
                    "Episodic Return: {}, Time Step {}".format(
                        info["episode"]["r"][0], t
                    )
                )
            ...
            s, _ = env.reset()
    env.close()
    save_dir = "data_stream_ac_{}_lr{}_gamma{}_lamda{}_entropy_coeff{}".format(
        env.spec.id, lr, gamma, lamda, entropy_coeff
    )
