...
try:
    ...
    logging.warning("Cannot import matplotlib and/or seaborn.Will not be able to render the environment.")
...
#####################################################################################################################
class Environment:
    def __init__(self, env_name, sticky_action_prob=0.1, difficulty_ramping=True, use_minimal_observation=True):
        env_module = import_module("envs.minatar." + env_name)
        self.random = np.random.RandomState()
        self.env_name = env_name
        self.env = env_module.Env(ramping=difficulty_ramping, use_minimal_observation=use_minimal_observation)
