...
class JBWEnv(gym.Env):
    ...
    def __init__(self, sim_config, reward_fn, render=False, f_type="obj"):
        ...
        self.feature_space = spaces.Box(
            low=np.zeros(self.t_size), high=np.ones(self.t_size)
        )
    ...
    def feature_picker(self, ftype="hash"):
        if ftype == "hash":
                ...
                for s in self.shape_set:
                    ids = (
                        np.bitwise_xor.reduce(
                            view_as_windows(obs_hash, s), axis=(2, 3)
                        ).flatten()
                        % self.t_size
                    )
            ...
            self.t_size = (len(self.hash_dict) - 1) * (
                self.sim_config.vision_range * 2 + 1
            ) ** 2
    ...
    def step(self, action):
        ...
        return (
            (self.vision_state, self.scent_state, self.feature_state),
            reward,
            done,
            {},
        )
    ...
    def reset(self):
        ...
        if self._render:
            ...
            self._painter = MapVisualizer(
                self._sim, self.sim_config, bottom_left=(-70, -70), top_right=(70, 70)
            )
        self.vision_state = self._agent.vision()
        self.scent_state = self._agent.scent()
        self.feature_state = self.get_features(self.vision_state)
